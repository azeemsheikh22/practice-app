...
const AlertPopup = ({
  ...
  return (
    <div
      className="fixed bottom-4 overflow-hidden pb-3 right-4 flex flex-col items-end"
      ...
      <AnimatePresence>
        {alerts.length > 0 && (
          <motion.div
            ...
            onMouseEnter={() =>
              onAlertHover && onAlertHover(alerts[alerts.length - 1].alm_id)
            }
            onMouseLeave={() => onAlertHover && onAlertHover(null)}
          >
            <div className="flex items-center gap-3 mb-1">
              <div>
                {alertTypeIcons[alerts[alerts.length - 1].alm_type] || (
                  <AlertCircle size={20} />
                )}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div
                  className="text-[11px] text-gray-200 max-w-[200px] overflow-hidden text-ellipsis"
                  style={{
                    display: "-webkit-box",
                    WebkitLineClamp: 2,
                ...
                >
                  {alerts[alerts.length - 1].carname} @{" "}
                  {alerts[alerts.length - 1].location}
                </div>
              </div>
              <button
            ...
            <div className="flex justify-between items-center mt-1">
              <button
                ...
                onClick={() =>
                  onViewAlert && onViewAlert(alerts[alerts.length - 1])
                }
                style={{ pointerEvents: "auto" }}
              >
                View Alert
            ...
            {alerts.length > 1 && (
              <div
                className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-xs font-semibold px-3 py-0.5 rounded-full shadow-lg border border-white select-none"
