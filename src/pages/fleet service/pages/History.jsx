...
const History = () => {
  ...
  // Service history data based on the image
  const historyData = [
    {
      id: 'ALB-545-1',
      vehicleName: 'ALB-545',
    ...
    },
    {
      ...
      vehicleName: 'ALB-545',
      type: 'tyre-burst',
      serviceName: 'Tyre Burst',
      odometer: '0 Kms',
      engineHours: '',
      dateCompleted: '17/02/2024',
      cost: '15200',
      status: 'completed'
    }
  ];
  ...
  const vehicleOptions = Array.from(
    new Set(historyData.map((history) => history.vehicleName))
  ).map((v) => ({ value: v, label: v }));

  const serviceOptions = Array.from(
    new Set(historyData.map((history) => history.type))
  ).map((v) => ({ value: v, label: v }));
  ...
  const exportColumns = [
    ...
    { key: 'cost', header: 'Cost(Rs)' }
  ];

  const handleExport = () => {
    const csv = arrayToCSV(historyData, exportColumns);
    downloadCSV(csv, 'service-history.csv');
  };
  ...
  const getServiceIcon = (type) => {
    switch (type) {
      case 'oil-change':
        return '/icons/oil-change.png';
      case 'denting':
        return '/icons/denting.png';
      case 'tyre-burst':
        return '/icons/tyre.png';
      default:
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-sm text-gray-600">
              View completed service records and maintenance history
            </p>
          </div>
          <div className="flex-shrink-0">
            <button className="btn btn-primary btn-sm bg-gradient-to-r from-[#25689f] to-[#1F557F] hover:from-[#1F557F] hover:to-[#184567] border-none text-white shadow-md hover:shadow-lg transition-all duration-200 flex items-center px-3 py-1.5 text-sm h-auto min-h-0 font-semibold">
              Create One Time Service Record
            </button>
          </div>
        </div>
      ...
      <div className="bg-white shadow-lg rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-4 py-2.5 border-b border-gray-200 bg-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <h3 className="text-md font-semibold text-gray-900">
            Showing {historyData.length} of {historyData.length} Service Records
          </h3>
          <button
                  ...
                  <tr key={history.id}>
                    ...
                    <td className="px-3 py-2 text-sm text-gray-700">
                      <span className="flex items-center">
                        <img
                          src={getServiceIcon(history.type)}
                          alt={history.type}
