...
const Reminders = () => {
  ...
  const reminderData = [
    {
      id: '121211651651651',
      vehicleName: '121211651651651',
      type: 'oil-change',
      serviceName: 'test',
      daysToGo: '',
      distanceToGo: '89296 kms to go',
      engineHoursToGo: '',
      status: 'upcoming'
    },
    {
  ...
  const vehicleOptions = Array.from(
    new Set(reminderData.map((reminder) => reminder.vehicleName))
  ).map((v) => ({ value: v, label: v }));

  const serviceOptions = Array.from(
    new Set(reminderData.map((reminder) => reminder.type))
  ).map((v) => ({ value: v, label: v }));
  ...
  const exportColumns = [
    ...
    { key: 'daysToGo', header: 'Days to Go' },
    { key: 'distanceToGo', header: 'Distance to Go' },
    { key: 'engineHoursToGo', header: 'Engine Hours to Go' },
    { key: 'status', header: 'Status' }
  ];

  const handleExport = () => {
    const csv = arrayToCSV(reminderData, exportColumns);
    downloadCSV(csv, 'service-reminders.csv');
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-sm text-gray-600">
              View and manage your vehicle service reminders
            </p>
          </div>
      ...
      <div className="bg-white shadow-lg rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-4 py-2.5 border-b border-gray-200 bg-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <h3 className="text-md font-semibold text-gray-900">
            Showing {reminderData.length} of {reminderData.length} Service Reminders
          </h3>
          <button
              ...
              {loading ? (
                ...
                reminderData.map((reminder) => (
                  <tr key={reminder.id}>
                    ...
                    <td className="px-3 py-2 text-sm text-gray-700">
                      {reminder.distanceToGo}
                    </td>
                    <td className="px-3 py-2 text-sm text-gray-700">
                      {reminder.engineHoursToGo}
                    </td>
                    <td className="px-3 py-2 text-sm text-gray-700">
